if (!window.NSwitchTabs) {
    ...
    var NSwitchTabs = (function () {
         ...
         *
         * Version 2.3.0:
         * Добавлена проверка на существование window.NSwitchTabs
         *
         * Version 2.4.0:
        ...
        NSwitchTabs.prototype = {
            ...
            init: function ($container, $containerView, params) {
                ...
                try {
                    ...
                    if (that._config.useLocalStorage && localStorage &&
                     (localStorage.getItem(that._config.keyLocalStorage) !== null)) {

                        that.currTabName = localStorage.getItem(that._config.keyLocalStorage);

                        if (!that.$buttons
                                .filter('[data-' + that._config.buttonsDataAttrName + '="' +
                                 that.currTabName + '"]').length) {
                            throw new Error('Такой вкладки не существует');
                        }

                    } else {

                        that.currTabName = $(that.$buttons[0]).data(that._config.buttonsDataAttrName);
                    }
                ...
                that.$buttons.triggerHandler('tabs.change', [that.currTabName, null]);
            ...
            binding: function () {
                ...
                that.$container.on('click', '[data-' + that._config.buttonsDataAttrName + ']', function (event) {
                    ...
                    if (that.currTabName && tabName !== that.currTabName) {
                        $(this).triggerHandler('tabs.change', [tabName, prevTabName]);
                        event.preventDefault();
                    }

                });

                that.$buttons.on('tabs.change', function (event, tabName, prevTabName) {
            ...
            selectButton: function (tabName) {
                ...
                that.$buttons.filter('[data-' + that._config.buttonsDataAttrName + '="' + tabName + '"]')
                    .addClass(that._config.activeButton);
